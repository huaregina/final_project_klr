...
```{python}
scatter = alt.Chart(df_vacant).mark_point(size=0.3,filled=True, color='blue').encode(
    longitude='longitude',
    latitude='latitude'
).properties(
...
combined_map = (crime_rate_map + scatter).properties(
    title={
...
vacant_land_counts = df_vacant.groupby('zip_code').size().reset_index(name='vacant_land_count')
...
```{python}
gdf_pop[gdf_pop['total_population']==0]
```
```{python}
