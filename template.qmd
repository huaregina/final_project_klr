...
```{python}
print(f"The total area of all the city owned vacant land is {sum(df_coc['Sq. Ft.'])}")  # 13,318,362 Sq. Ft.
```
...
scatter_city = alt.Chart(df[df['type']=='City-Owned']).mark_point(size=0.5, filled=True, color='purple').encode(
    longitude='longitude',
    latitude='latitude',
    tooltip=['latitude:Q', 'longitude:Q', 'chicago_community_area_name:N', 'zip_code:N' ]
).project(
...
scatter_private = alt.Chart(df[df['type']=='Private']).mark_point(size=0.5, filled=True, color='green').encode(
    longitude='longitude',
    latitude='latitude',
    tooltip=['latitude:Q', 'longitude:Q', 'chicago_community_area_name:N', 'zip_code:N' ]
).project(
...
base_map = alt.Chart(alt.Data(values=json.loads(geojson_data)['features'])).mark_geoshape(
    ...
    type='identity'  # Use Albers USA projection for visualization
).properties(
    width=600,
    height=400
)
...
gdf = gpd.read_file('./CommaArea/CommAreas.shp')

# Read Social Economic file
df_se = pd.read_csv('./socioeconomic.csv')
